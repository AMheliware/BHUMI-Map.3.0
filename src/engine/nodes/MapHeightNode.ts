...
export class MapHeightNode extends MapNode 
{
	 ...
	 */
	public constructor(parentNode: MapHeightNode | null = null, mapView: MapView | null = null, location: number = MapNode.root, level: number = 0, x: number = 0, y: number = 0, geometry: BufferGeometry = MapHeightNode.geometry, material: Material = new MeshPhongMaterial({wireframe: false, color: 0xffffff})) 
	{
	...
	{
		// // console.log('Load Texture Map Height Node');
		...
		try {
			...
			try {
				texture.image = await this.mapView!.mapBoxProvider!.fetchTile(this.level, this.x, this.y);
			} catch (error) {
				// // console.log('load Texture error mapbox', error);
			}
		}
	...
	public async loadHeightGeometry(): Promise<any> 
	{
		...
		let image;
		try {
			image = await this.mapView!.heightProvider.fetchTile(this.level, this.x, this.y);
		} catch (error) {
			// // console.log('loadHeightGeometry', error);
		}
		...
		this.geometry = new MapNodeHeightGeometry(1, 1, this.geometrySize, this.geometrySize, true, 10.0, imageData, true, this.mapView?.heightProvider.elevExaggeration);
